...
class TextEdit(QTextEdit):
    ...
    def inputMethodEvent(self, event):
        ...
        if commitString:         
            at = InputMethodTextUndoCommand(
                self.document(),
                commitString,
                self.textCursor().position() ,
                undotext="inputMethod")
            self.pushUndoCommand(at)
    ...
    def showParagraphDialog(self):
        ...
        if d.exec() == QDialog.Accepted:
            ...
            c = SetBlockFormatByParagraphDialog(
                self.document(), textIndent, indent, rightIndent, lineHeight, bottomMargin, alignment,
                textIndent, indent, rightIndent, float(d.lineHeightComboBox.currentText()), 10 if d.tenPointPlusCheckBox.isChecked() else 0,
                newAlignment,
                "SetBlockFormatByParagraphDialog")
            self.pushUndoCommand(c)
    ...
    def setNoWrap(self):
        ...
        setNoWrapUndoCommand = SetWrapUndoCommand(
            self.document(),
            QTextEdit.NoWrap,
            self.lineWrapMode(),
            "SetNoWrapUndoCommand"
            )
        self.pushUndoCommand(setNoWrapUndoCommand)
        
    def setWidgetWidth(self):

        setWidgetWidthUndoCommand = SetWrapUndoCommand(
            self.document(),
            QTextEdit.WidgetWidth,
            self.lineWrapMode(),
            "SetWidgetWidthUndoCommand"
            )
        self.pushUndoCommand(setWidgetWidthUndoCommand)
        
        
    def setFixedPixelWidth(self):

        setFixedPixelUndoCommand = SetWrapUndoCommand(
            self.document(),
            QTextEdit.LineWrapMode.FixedPixelWidth,
            self.lineWrapMode(),
            "SetFixedPixelWidthUndoCommand"
            )
        self.pushUndoCommand(setFixedPixelUndoCommand)      

    def setAlignment(self, bool):
        tc = self.textCursor()
        
        blockFormat = tc.blockFormat()
        alignToolButton = self.sender()
        ...
        if bool:
            ...
            name = alignToolButton.objectName()
            setAlignmentUndoCommand = SetAlignmentUndoCommand(self.document(),
                                                              dic[name],
                                                              "SetAlignmentUndoCommand")
            self.pushUndoCommand(setAlignmentUndoCommand)
        
    def setBottomMargin(self, bool):
        
        setBottomMarginUndoCommand = SetBottomMarginUndoCommand(
            self.document(), bool, "SetBottomMarginUndoCommand"
            )
        self.pushUndoCommand(setBottomMarginUndoCommand)

        
    def setLineHeight(self, height):

        setLineHeightUndoCommand = SetLineHeightUndoCommand(
            self.document(), height, "SetLineHeightUndoCommand"
            )
        self.pushUndoCommand(setLineHeightUndoCommand)
    ...
    def insertTextList(self, label):
        ...
        insertListUndoCommand = InsertListUndoCommand(
            self.document(), lf.style(), "InsertListUndoCommand"
            )
        self.pushUndoCommand(insertListUndoCommand)

    def moveTabPosition(self, float):

        tabPointer = self.sender()
        
        moveTabPositionUndoCommand = MoveTabPositionUndoCommand(
                        self.document(),  tabPointer, fr, to, "MoveTabPositionUndoCommand"
                        )
        self.pushUndoCommand(moveTabPositionUndoCommand)

    def plusIndent(self):

        plusIndentUndoCommand = PlusIndentUndoCommand(
                                    self.document(), "plus", "PlusIndentUndoCommand"
                                    )
        self.pushUndoCommand(plusIndentUndoCommand)

    def minusIndent(self):
        minusIndentUndoCommand = MinusIndentUndoCommand(
                                    self.document(), "minus", "MinusIndentUndoCommand"
                                    )
        self.pushUndoCommand(minusIndentUndoCommand)
    ...
    def setTextIndent(self, float):
        setTextIndentUndoCommand = SetTextIndentUndoCommand(
            self.document(), float, "SetTextIndentUndoCommand"
            )
        self.pushUndoCommand(setTextIndentUndoCommand)

    def setIndent(self, float):

        setIndentUndoCommand = SetIndentUndoCommand(
            self.document(), float, "SetIndentUndoCommand"
            )
        self.pushUndoCommand(setIndentUndoCommand)

    def setRightIndent(self, float):
        setRightIndentUndoCommand = SetRightIndentUndoCommand(
            self.document(), float, "rightIndent"
            )
        self.pushUndoCommand(setRightIndentUndoCommand)

    def setTabPosition(self, tab, float):
        
        setTabPositionsUndoCommand = SetTabPositionUndoCommand(self.document(),
                                        self.sender(), 
                                        float,
                                        "TabPositionsUndoCommand")
        self.pushUndoCommand(setTabPositionsUndoCommand)
    ...
    def cut(self):
        ...
        if tc.hasSelection():
            ...
            cutUndoCommand = CutUndoCommand(
                self.document(),  "CutUndoCommand"
                )
            self.pushUndoCommand(cutUndoCommand)
    ...
    def paste(self):
            ...
            pasteUndoCommand = PasteUndoCommand(
                self.document(), "PasteUndoCommand"
                )
            self.pushUndoCommand(pasteUndoCommand)
    ...
    def keyPressEvent(self, event):
        ...
        if isBack or isDelete:
            deleteTextUndoCommand =DeleteTextUndoCommand(self.document(), isBack, "DeleteTextUndoCommand")
            self.pushUndoCommand(deleteTextUndoCommand)
            return
        ...
        if event.text() and not event.key() == Qt.Key_Backspace and not event.key() == Qt.Key_Delete:

            if event.modifiers() != Qt.ControlModifier:
                at = AppendTextUndoCommand(self.document(), event.text(), self.textCursor().position(), self.tr("AppendText"))
                self.pushUndoCommand(at)
    ...
    def pushUndoCommand(self, undocommand):
    ...
    def showTimeDialog(self):
        ...
        if d.exec() == QDialog.Accepted:
            ...
            insertDateAndTimeUndoCommand = InsertDateAndTimeUndoCommand(
                self.document(), item.text(), "InsertDateAndTimeUndoCommand")
            self.pushUndoCommand(insertDateAndTimeUndoCommand)
    ...
    def createMimeDataFromSelection(self):
        ...
        if chr(0xfffc) not in tc.selectedText():
             self.mimeData =  super().createMimeDataFromSelection()
    ...
    def loadImage(self):
        ...
        if f:
            loadImageUndoCommand = LoadImageUndoCommand(self.document(),
                                                    QImage(f),
                                                    "LoadImageUndoCommand")
            self.pushUndoCommand( loadImageUndoCommand)
    ...
    def changeImage(self):
        ...
        if f:
            i = QImage(f)
            changeImageUndoCommand = ChangeImageUndoCommand(
                                self.document(), i, "ChangeImageUndoCommand"
                                )
            self.pushUndoCommand( changeImageUndoCommand)
    ...
    def changeImageSize(self):#new↓
        ...
        if d.exec() == QDialog.Accepted:
            ...
            p = label.pixmap()
            changeImageSizeByDialogUndoCommand =  ChangeImageSizeByDialogUndoCommand(
                self.document(), h, v, aspectRatio, "ChangeImageSizeUndoCommand"
                )
            self.pushUndoCommand(changeImageSizeByDialogUndoCommand)
    ...
    def setHighlightByAction(self, label):
        ...
        highlightColorUndoCommand = HighlightColorUndoCommand(
            self.document(), color, "SetHighlightColorUndoCommand"
            )
        self.pushUndoCommand(highlightColorUndoCommand)
        

    def setTextColorByAction(self, label):
        textColorUndoCommand = TextColorUndoCommand(
            self.document(), color, "SetTextColorUndoCommand"
            )
        self.pushUndoCommand(textColorUndoCommand)

    def setBold(self, bool):

        setBoldUndoCommand = SetBoldUndoCommand(
            self.document(), bool, "SetBoldUndoCommand")
        self.pushUndoCommand(setBoldUndoCommand)

    def setItalic(self, bool):

        setItalicUndoCommand =SetItalicUndoCommand(
            self.document(), bool, "SetItalicUndoCommand")
        self.pushUndoCommand(setItalicUndoCommand)

    def setStrikeOut(self, bool):
        setStrikeOutUndoCommand =SetStrikeOutUndoCommand(self.document(),
                                                         bool,
                                                         "SetStrikeOutUndoCommand")
        self.pushUndoCommand(setStrikeOutUndoCommand)

    def setUnderline(self, bool):
        setUnderlineUndoCommand =SetUnderlineUndoCommand(
                                                        self.document(),
                                                         bool,
                                                         "SetUnderlineUndoCommand")
        self.pushUndoCommand(setUnderlineUndoCommand)

    def setSuperScript(self, bool):
        setVerticalAlignmentUndoCommand = SetVerticalAlignmentUndoCommand(
            self.document(),
            AlignSuperScript if bool else NormalScript,
            "setSuperScriptUndoCommand")
        self.pushUndoCommand(setVerticalAlignmentUndoCommand)

    def setSubScript(self, bool):
        setVerticalAlignmentUndoCommand = SetVerticalAlignmentUndoCommand(self.document(),
                                                                          AlignSubScript if bool else NormalScript,
                                                                          "setSubScriptUndoCommand")
        self.pushUndoCommand(setVerticalAlignmentUndoCommand)
        
    def setCurrentFont(self, font):

        setCurrentFontUndoCommand =  SetCurrentFontUndoCommand(
            self.document(), font, "SetCurrentFontUndoCommand"
            )
        self.pushUndoCommand(setCurrentFontUndoCommand)

    def crementPointSize(self): 
        tc = self.textCursor()
        s = self.sender()

        crementUndoCommand = CrementFontSizeUndoCommand(
            self.document(), s.objectName(), "CrementUndoCommand")
        self.pushUndoCommand(crementUndoCommand )
